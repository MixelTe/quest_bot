...
def started(bot: Bot, user: User):
    ...
    for text in [
        ...
        "Подготовил для вас подарочек \\-  *250к тонн тротила* 😂 Они спрятаны в тайнике, вход в который открывается ровно в 13:00 всего на пять минут\\. А потом \\- все, всему конец\\!",  # noqa E501
...
def task10(bot: Bot, user: User):
    task = "Теперь он здорово проголодался и ищет, где поесть. Нашел какой-то ресторан со странным названием типа «чая больше нет». Поторопитесь и спасите меня наконец! Когда найдете это место, введите весь код целиком, я впишу в командный блок код и ваши имена в вайт лист.\nПомните, что вход откроется ровно в 13:00 только на пять минут"  # noqa E501
...
def quest_end(bot: Bot, user: User):
    ...
    msg.blockquote("Вход откроется ровно в 13:00 только на пять минут")
    forward(bot.sendMessage(msg))
